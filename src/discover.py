...
import netaddr
import models
from sysobjectid import manufacturers


def gater_device_data(address, snmp_data):
    if snmp_data["version"] == 2:
        session = Session(hostname=address, community=snmp_data["community"], version=2, use_sprint_value=True)
    ...
    device_data = {
        "hostname": device_name_oid.value
    }
...
def process_interfaces(session):
    ...
    for address in addresses:
        if addresses[address]["if_oid"] in interfaces:
            interfaces[addresses[address]["if_oid"]]["address"] = str(netaddr.IPNetwork(f'{addresses[address]["address"]}/{addresses[address]["netmask"]}').cidr)
...
device_data = gater_device_data("10.67.200.1", snmp_data)
